...
```{r}
svm1 <- svm(target~., data=z_completetrain, method="C-classification", scale = FALSE, kernal="polynomial", cost=40, degree=5) # polynomial kernel
...
correct/nrow(z_completetest) # 90.97% accuracy 
...
correct/nrow(z_completetrain) # 96.15% accuracy 
```
...
```{r}
svm2 <- svm(target~., data=z_normaltrain, method="C-classification", scale = FALSE, kernal="polynomial", cost=40, degree=5) # polunomial kernel 
...
correct/nrow(z_normaltest) # 89.6% accuracy 
# Accuracy for training 
...
correct/nrow(z_normaltrain) # 93.1% accuracy 
...
```{r}
svm3 <- svm(target~., data=z_completetrain, method="C-classification", scale = FALSE, kernal="radial", cost=5)
...
table(truth3, pred3) # More mis-classified throughout (for all variables)
# Accuracy for testing 
correct <- 0
for (i in 1:5) {
  correct <- correct + table(truth3, pred3)[i,i]
}
correct/nrow(z_completetest) # 87.21% accuracy 
# Accuracy for training 
...
correct/nrow(z_completetrain) # 92.45% accuracy 

```
This is the third best accuracy out of all svm testing for both test and train data sets. 

# All features for z normal data set using radial kernel 
```{r}
svm4 <- svm(target~., data=z_normaltrain, method="C-classification", scale = FALSE, kernal="radial", cost=5) # radial kernel 
...
correct/nrow(z_normaltest) # 85.9% accuracy 
# Accuracy for training 
...
correct/nrow(z_normaltrain) # 88.8% accuracy 

```
This is the fourth best accuracy out of all svm testing for both test and train data sets.
